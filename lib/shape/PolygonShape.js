...
PolygonShape.prototype.rayCast = function(output, input, xf, childIndex) {
  ...
  for (var i = 0; i < this.m_count; ++i) {
    ...
    var numerator = Vec2.dot( this.m_normals[i], p.set( v1.set(this.m_vertices[i]) ).sub(p1) );
    ...
    if (denominator == 0.0) { // ray is parallel to poligon's side
      ...
      v2.set(i < this.m_count - 1 ? this.m_vertices[i + 1] : 0);
      var intersects = Vec2.dot(v1.sub(p), v2.sub(p)) < 0;
      if (!intersects) {
        continue;
      }
